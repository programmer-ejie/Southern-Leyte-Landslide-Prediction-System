...
function applyTheme() {
    ...
    try {
        window.parent.document.documentElement.dataset.theme = isDark ? 'dark' : 'light';
    } catch (_) {}
    // Landing icons
    ...
    // Update charts after the template finishes initializing chart state.
...
function swTab(t) {
    ...
    document.getElementById('fSignup')?.style && (document.getElementById('fSignup').style.display = isL ? 'none' : 'block');
    document.getElementById('tabLogin').classList.toggle('on', true);
    document.getElementById('tabSignup')?.classList.toggle('on', !isL);
    document.getElementById('loginErr').style.display = 'none';
    if (document.getElementById('signupErr')) document.getElementById('signupErr').style.display = 'none';
}
...
function setLoading(btnId, loading) {
    ...
    if (loading) {
        ...
        else btn.innerHTML = 'Request Access <i class="fa-solid fa-arrow-right ms-1 fa-sm"></i>';
    }
}
...
async function doLogin() {
    ...
    try {
        ...
        if (!res.ok) {
            showErrLogin(data.detail || 'Invalid email or password.');
            return;
        }
        loginSuccess(data);
    } catch (err) {
        showErrLogin('Cannot reach the login server. Please start the backend and try again.');
    } finally {
...
function loginSuccess(session) {
    ...
    try {
        const storage = window.top?.localStorage || localStorage;
        storage.setItem(AUTH_USER_KEY, JSON.stringify(session.user));
        storage.setItem(AUTH_TOKEN_KEY, session.accessToken);
    } catch (_) {
...
async function sendChat() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            appendMsg('Ã¢Å¡Â Ã¯Â¸Â Sorry, I had trouble connecting. Please check your API key or try again.', 'ai');
        }
    } catch (e) {
        removeTyping(typingId);
        appendMsg('Ã¢Å¡Â Ã¯Â¸Â Network error. Please ensure you are connected to the internet.', 'ai');
    }
...
function appendMsg(text, role) {
    ...
    <div class="msg-time" style="align-self:${role==='ai'?'flex-start':'flex-end'};padding:0 4px">${role==='ai'?'NexusAI':'You'} Ã‚Â· ${time}</div>`;
...
function clearChat() {
    ...
    <div class="d-flex flex-column gap-1">
      <div class="msg msg-ai">Ã°Å¸â€˜â€¹ Hi! I'm your NexusAI assistant. I can help you with support analytics, agent configuration, automation workflows, and business insights. What would you like to know?</div>
      <div class="msg-time" style="align-self:flex-start;padding-left:4px">NexusAI Ã‚Â· Just now</div>
    </div>`;
}
...
const activities = [
    ...
    ['#34d399', 'Refund processed automatically Ã¢â‚¬â€ $29.99'],
    ['#fbbf24', 'Escalation: account access issue Ã¢â€ â€™ human agent'],
    ['#60a5fa', 'Data sync completed Ã¢â‚¬â€ 28 CRM records updated'],
